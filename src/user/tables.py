...
ACTION_TEMPLATE = """
{% if details_url %}
    <a href="{% url details_url record.pk %}" class="btn btn-sm btn-info">
        اطلاعات
    </a>
{% endif %}

{% if update_url %}
    <a href="{% url update_url record.pk %}" class="btn btn-sm btn-warning">
        ویرایش
    </a>
{% endif %}

{% if delete_url %}
    <a href="{% url delete_url record.pk %}" class="btn btn-sm btn-danger">
        حذف
    </a>
{% endif %}
"""
...
class RepliedTutorialCommentTable(tables.Table):
    ...
    tutorial = tables.TemplateColumn(
        _tutorial_link_template, verbose_name="آموزش"
    )
    parent_comment = tables.TemplateColumn(
        _parent_comment_link_template, verbose_name="پاسخ به"
    )

    actions = tables.TemplateColumn(
        _replied_comment_btn_template, orderable=False, verbose_name="اقدام"
    )
...
class TutorialCommentUserRelationsTable(tables.Table):
    ...
    _tutorial_link_template = (
        '<a href="{% url "learning:tutorial" '
        'record.comment.tutorial.slug %}">'
    ...
    comment = tables.TemplateColumn(
        _comment_link_template, verbose_name="دیدگاه"
    )
