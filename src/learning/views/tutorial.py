...
from learning.models import Tutorial, Category, TutorialView


def get_tutorial_by_categories(
    ...
    """
    tutorials = (
        Tutorial.objects.filter(categories__in=categories)
        .only(*fields)
        .exclude(id=tutorial_id)
...
def get_related_tutorials(
    tutorial: Tutorial, fields: tuple, tutorial_count: int = 5
) -> list[Tutorial]:
    """
...
def tutorial_details_view(request: HttpRequest, slug: str):
    ...
    comments = tutorial.comments.select_related(
        "parent_comment", "user"
    ...
    liked_by_current_user = (
        ...
        and tutorial.likes.filter(user_id=request.user.id).exists()
    )
