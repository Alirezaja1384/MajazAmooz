...
AllowReplies.addEventListener(
    "click",
    () =>
        (NotifyReplies.disabled = !(NotifyReplies.checked =
            AllowReplies.checked))
);

function ReplyTo(commentId, comment_title) {
    document.getElementById("Comment_Reply_To").value = commentId;
    document.getElementById("ReplyTo-Alert-CommentTitle").innerHTML =
        "در حال پاسخ به " + comment_title;
    $("#ReplyTo-Alert").show();
...
document
    .getElementById("TutorialSubmitCommentForm")
    .addEventListener("submit", function (e) {
        e.preventDefault();

        let replyTo = parseInt(
            document.getElementById("Comment_Reply_To").value
        );
    ...
    fetch("/ajax/tutorial/upvote", {
        ...
        .then((content) => {
            if (parseInt(content["status"]) === 0)
                ...
                let nextUpVoteCount =
                    parseInt(upVotes.innerHTML) + parseInt(content["status"]);
    ...
    fetch("/ajax/tutorial/downvote", {
        ...
        .then((content) => {
            if (parseInt(content["status"]) === 0)
                ...
                let downVotes = document.getElementById(
                    "TutorialDownVotesCount"
                );
                let nextDownVoteCount =
                    parseInt(downVotes.innerHTML) + parseInt(content["status"]);
    ...
    fetch("/ajax/tutorial/like", {
        ...
        .then((content) => {
            if (parseInt(content["status"]) === 0)
                ...
                let nextLikesCount =
                    parseInt(likes.innerHTML) + parseInt(content["status"]);
...
function UpVoteTutorialComment(tutorialCommentId) {
    let commentUpVoteBTN = document.getElementById(
        "comment-upvote-btn-" + tutorialCommentId
    );
    ...
    fetch("/ajax/tutorial_comment/upvote", {
        ...
        .then((content) => {
            if (parseInt(content["status"]) === 0)
                ...
                let upVotes = document.getElementById(
                    "TutorialCommentUpVotesCount-" + tutorialCommentId
                );
                let nextUpVoteCount =
                    parseInt(upVotes.innerHTML) + parseInt(content["status"]);
                upVotes.innerHTML = nextUpVoteCount.toString();
...
function DownVoteTutorialComment(tutorialCommentId) {
    let commentDownVoteBTN = document.getElementById(
        "comment-downvote-btn-" + tutorialCommentId
    );
    ...
    fetch("/ajax/tutorial_comment/downvote", {
        ...
        .then((content) => {
            if (parseInt(content["status"]) === 0)
                ...
                let downVotes = document.getElementById(
                    "TutorialCommentDownVotesCount-" + tutorialCommentId
                );
                let nextDownVoteCount =
                    parseInt(downVotes.innerHTML) + parseInt(content["status"]);
                downVotes.innerHTML = nextDownVoteCount.toString();
...
function LikeTutorialComment(tutorialCommentId) {
    let commentLikeBTN = document.getElementById(
        "comment-like-btn-" + tutorialCommentId
    );
        ...
        .then((content) => {
            if (parseInt(content["status"]) === 0) {
                ...
                let likes = document.getElementById(
                    "TutorialCommentLikesCount-" + tutorialCommentId
                );
                let nextLikeCount =
                    parseInt(likes.innerHTML) + parseInt(content["status"]);
